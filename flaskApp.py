...
#         elif feature == 'LST':
#             collection = ee.ImageCollection('MODIS/006/MOD11A1').filterBounds(roi).filterDate(month_start, month_end)
#             image = collection.select('LST_Day_1km').mean().multiply(0.02).subtract(273.15)  # Convert to Celsius

#         value = image.reduceRegion(reducer=ee.Reducer.mean(), geometry=roi, scale=1000, maxPixels=1e9).getInfo()

#         monthly_values.append({
#             'date': month_start,
#             'value': value['nd' if feature == 'NDVI' else 'sm' if feature == 'SM' else 'LST_Day_1km' if feature == 'LST' else '']
#         })
...
#     elif feature == 'LST':
#         collection = ee.ImageCollection('MODIS/006/MOD11A1').filterBounds(roi).filterDate(start_date, end_date)
#         image = collection.select('LST_Day_1km').mean().multiply(0.02).subtract(273.15)  # Convert to Celsius
